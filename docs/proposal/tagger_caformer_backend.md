...
Do **not** invent a new objective. `library/captioning/readback.py::TagReadback`
...
Keep in tree: `library/captioning/{anima_tagger, dbv4_backend, position_clauses,
...
(moves out of `train_common.py` into `library/captioning/`).
...
- **Name-space drift.** dbv4 uses danbooru names as of 2025-10; `rules.yaml`
  ...
  `library/captioning/dbv4_backend.py::align_vocab` is the single join point
