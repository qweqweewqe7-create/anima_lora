...
def main() -> None:
    ...
    cols: list[tuple[str, str, dict, dict, Path]] = []  # (run, spec, floors, per_img, sheetdir)
    for run in runs:
    ...
    for run, spec, fl, _, _ in cols:
        for f, m in fl.items():
            g = "<span class=ok>PASS</span>" if m["gate"] else "<span class=bad>fail</span>"
            h.append(
    ...
    h.append("</table>")
    h.append(f"<p>{len(images)} images (union of each prompt's worst-recall sheets); per-image counts at floor {a.floor}.</p>")
    ...
    for img in images:
        ...
        for run, spec, _, pi, sdir in cols:
            ...
            h.append(f"<div class=cell><div class=cap>{html.escape(short(spec))} — {cap}</div>")
            if f.exists():
